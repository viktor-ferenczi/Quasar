...
Constants:
- `MaxPointsCeiling = 1000`, `MaxPointsFloor = 10`, `MaxProfilerEntrySeriesPerChart = 20`

Constructor:
- `AnalyticsSeriesService(MetricsStoreService store, ProfilerStoreService profilerStore)`

Methods:
- `Build(long fromUnix, long toUnix, IReadOnlyList<string> servers, IReadOnlyList<string> metricKeys, int maxPoints) : AnalyticsSeriesResponse` — validates range/inputs, resolves scalar metrics via `AnalyticsMetrics.Find`, profiler timing metrics via `ProfilerAnalyticsMetrics.Find`, and top-entry metrics via `ProfilerEntryAnalyticsMetrics.Find`; buckets values, drops empty scalar buckets, and returns profiler chart DTOs even when their current series are empty.
...
- `BuildProfilerEntryCharts(...)` — reads normalized profiler snapshots, buckets top grid/entity entries by per-frame cost, ranks the busiest labels over the requested range, and emits up to 20 chart series per panel.
...
- `ResolveEntryValue(ProfilerEntrySnapshot)` / `BuildEntrySeriesLabel(...)` — select per-frame timing and create stable `server / entry` labels for top-list charts.
...
- `EntryBuckets` — per-entry bucket sums/counts plus total rank score.

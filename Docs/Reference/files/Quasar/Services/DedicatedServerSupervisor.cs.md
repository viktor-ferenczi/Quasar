...
| `StartServerAsync(...)` | Guarded by `StartInProgress`; resolves runtime, prepares files, rotates the active stdout/stderr log slot, spawns the process with full env vars (including agent reconnect/shutdown/profiler mode), prepends Linux native-library search paths to `LD_LIBRARY_PATH` when the resolver found them, optionally logs launch diagnostics when the server definition enables `LogLaunchEnvironment`, applies startup priority, starts stdout/stderr pumps. |
...
`_preserveManagedServersOnShutdown` defaults from `WebServiceOptions.PreserveManagedServersOnShutdown`: managed servers are left running on a normal Quasar stop (they are detached via Magnetar `-daemon`/setsid) and reconnect when Quasar returns; the persisted PID snapshot is how the next worker re-adopts them. `StartInProgress` closes a TOCTOU race where two concurrent reconciles could both launch a process and collide on the port. Process priority applies in two phases (`StartupProcessPriority` at launch, `ReadyProcessPriority` once healthy/agent-online); on Linux via `renice -n {nice} -p {pid}`. Linux native-library search paths come from `ResolvedDedicatedServerRuntime.NativeLibrarySearchPaths` and are prepended to the inherited `LD_LIBRARY_PATH`, mainly to expose SteamCMD's `linux64/steamclient.so` runtime to Steam GameServer init on headless hosts. Per-server launch diagnostics are UI-owned and persisted on the server definition, not in appsettings. Agent profiler mode is global web-service configuration and is forwarded through `QUASAR_AGENT_PROFILER_MODE` at process start. Simulation-progress health re-baselines (skips judgement) during active world saves. State persists with a 150 ms debounce on every change and synchronously on shutdown/drain. DS log retention is enforced on Quasar-captured stdout/stderr archives, not Magnetar-internal files.

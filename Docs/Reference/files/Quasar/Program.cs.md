...
- Branding uploads are served via `PhysicalFileProvider` at `/branding` from `MagnetarPaths.GetQuasarBrandingDirectory()` (outside the build-time static-asset manifest and outside release folders). The standalone auth fallback pages embed their own CSS and static Quasar logo/favicon because they run outside the Blazor shell.

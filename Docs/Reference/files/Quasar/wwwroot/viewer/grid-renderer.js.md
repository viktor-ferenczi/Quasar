...
## Summary
Main renderer for grid, context-grid, and asteroid viewer scenes. It computes selected-grid floor alignment, creates one relative-view group per exported grid, clips only boundary-crossing context-grid block proxies/model triangles, logistics tracers, logistics masks, damaged-block masks, and voxel terrain to the selected primary grid floor X/Z footprint, renders toggleable logistics and damage overlays with through-wall model masks/fallback masks, including voxel deformation aura chunks filtered by per-sample modified masks, batches/proxies fully-inside blocks under their owning grid transform, builds capped in-scene light groups, resolves local Space Engineers models/textures from Content and Mods folders, applies Space Engineers-style paint/transparency/LCD behavior, progressively rebuilds per-grid model layers while assets load, and procedurally meshes voxel content/material chunks when present.

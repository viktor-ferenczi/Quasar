...
LCD text surfaces are treated the same way: the agent sends only text-surface metadata and logical texture paths for selected LCD images or programmable-block sprite layers. The browser matches LCD surface material names from the block definition to parsed MWM material groups and replaces those materials with either the selected local LCD image texture or a generated canvas texture for text/sprite composition. LCD replacement materials render in a dedicated late layer with depth writes disabled and a small polygon offset so screen textures are not hidden by nearby or coplanar block geometry. Static LCD images load from the selected local `Content` folder; text content, font/color settings, and programmable-block sprite text layers are sent as metadata so the browser can render characters without receiving the game's offscreen render target. Script-generated texture sprites use their logical LCD texture paths and are loaded locally when present. Dedicated-server builds do not recreate built-in client LCD scripts pixel-for-pixel, so surfaces whose visible output exists only as a client-side generated texture may fall back to their background/text metadata.

Applied block paint is rendered client-side from the scene `colourMaskHsv` metadata. Instanced model batches carry paint through per-instance color data instead of allocating a repeated per-vertex paint buffer for every rendered block. For textured models, the viewer uses Space Engineers-style color masking: base color comes from `ColorMetalTexture`/diffuse textures, paint strength comes from `AddMapsTexture`/extension-map alpha, and `*_alphamask.dds` is treated only as alpha/cutout data rather than a paint mask. When the selected local Content folder contains `Data/AssetModifiers/ArmorModifiers.sbc`, the browser resolves block `skinSubtypeId` values to the same material/location texture overrides used by the game client and swaps `ColorMetal`, `NormalGloss`, `Extensions`, and `Alphamask` paths before loading material textures. Alpha-masked model techniques are rendered as depth-writing cutouts instead of blended transparent sheets, and glass/holo/shield techniques render as blended materials with depth writes disabled to reduce self-overlap artifacts. `GLASS` mesh parts use transparent-material texture/color data from `TransparentMaterials.sbc` and do not consume the block paint mask, so painted window frames can still be colored while transparent glass keeps its normal glass color. When a local model cannot be parsed, proxy boxes use the same block paint color as a visual fallback.
